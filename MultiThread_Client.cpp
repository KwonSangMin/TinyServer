...
int main()
{
        ...
        /*int recv_length=0;
        while(recv_length<30)//클라이언트와 서버가 연결될 경우 서버의 자식 프로세스가 클라이언트에게 메세지를 보낸다. 하지만 네트워크 상황에따라 언제 패킷이 도착할지 알수없다. 또한, 클라이언트에서는 서버가 얼마만큼의 데이터를 보내는지
 알 수 없기 때문에 기존에 데이터양을 합의하거나, 전송 데이터 크기를 미리 전달해주어야한다. 그 이후에 원하는 데이터 크기만큼 수신이 될 때까지 무한루프를 돌려서 계속해서 패킷을 읽어오도록 해야한다.
  {
  ...
  printf("Client :: Message From Server -> %s\n",buffer);*/
	...
	scanf("%s", buffer);
        ...
        {
                printf("Close the Connection With %c\n",buffer[0]);
                break;
        }
